...
export function ProfileView({ identifier, own = false }: { identifier?: string; own?: boolean }) {
  ...
  async function follow() { if (!identifier) return; try { const result = await api<any>(`/api/usuarios/${identifier}/seguir`, { method: 'POST', token }); setData(current => ({ ...current, siguiendo: result.siguiendo ?? result.following, stats: { ...current.stats, seguidores: Number.isFinite(result.seguidores) ? result.seguidores : Math.max(0, current.stats.seguidores + ((result.siguiendo ?? result.following) ? 1 : -1)) } })); } catch (e) { Alert.alert('Error', e instanceof Error ? e.message : 'No se pudo seguir.'); } }
