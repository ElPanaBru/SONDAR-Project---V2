...
import { Avatar, Button, ErrorNotice, Field, Header, Loading, Screen, ui } from '@/components/sondar-ui';
...
export default function SettingsScreen() {
  ...
  const load = useCallback(async () => { setLoading(true); try { const [config, list] = await Promise.all([api('/api/usuarios/me/configuracion', { token }), api<any[]>('/api/usuarios/me/bloqueados', { token })]); setSettings({ ...defaults, ...config }); setBlocked(list); setError(''); } catch (e) { setError(e instanceof Error ? e.message : 'No se pudo cargar la configuración.'); } finally { setLoading(false); } }, [token]);
  ...
  return <Screen scroll><Header title="Configuración" subtitle="Cuenta, privacidad y experiencia" back /><ErrorNotice message={error} />
    <View style={styles.account}><Avatar name={settings.username || user?.email} size={58} /><View style={{ flex: 1 }}><Text style={ui.h2}>@{settings.username || user?.user_metadata?.username || 'usuario'}</Text><Text style={ui.muted}>{user?.email}</Text></View></View>
