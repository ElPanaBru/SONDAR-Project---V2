...
function ReelAction({ icon, label, onPress, active }: { icon: React.ComponentProps<typeof Ionicons>['name']; label: number | string; onPress: () => void; active?: boolean }) { return <Pressable onPress={onPress} style={styles.reelAction}><View style={[styles.actionCircle, active && styles.actionActive]}><Ionicons name={icon} size={24} color={active ? palette.orange : palette.text} /></View><Text style={styles.actionLabel}>{label}</Text></Pressable>; }

function CommentRow({ item, onLike, onReply, nested = false }: { item: Comment; onLike: (item: Comment) => void; onReply: (item: Comment) => void; nested?: boolean }) {
  return <View style={nested && styles.nestedComment}><View style={styles.comment}><Avatar uri={item.avatar} name={item.usuario} size={nested ? 30 : 36} /><View style={{ flex: 1 }}><Text style={styles.username}>{item.usuario} <Text style={styles.time}>{item.tiempo}</Text></Text><Text style={styles.commentText}>{item.texto}</Text><Pressable onPress={() => onReply(item)} hitSlop={8}><Text style={styles.replyAction}>Responder</Text></Pressable></View><Pressable onPress={() => onLike(item)} hitSlop={10} style={styles.commentLike}><Ionicons name={item.liked ? 'heart' : 'heart-outline'} size={17} color={item.liked ? palette.orange : palette.muted} /><Text style={styles.time}>{item.likes || 0}</Text></Pressable></View>{item.respuestas?.map(reply => <CommentRow key={reply.id} item={reply} onLike={onLike} onReply={onReply} nested />)}</View>;
}
