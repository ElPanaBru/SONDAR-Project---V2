...
const comunidadController = {
  actualizarMembresia: async (req, res) => {
    try {
      await asegurarEsquemaComunidades();
      const { comunidadId } = req.params;
      const comunidad = await pool.query('SELECT id FROM comunidades WHERE id = $1 AND activa = true', [comunidadId]);
      if (!comunidad.rowCount) return res.status(404).json({ error: 'Comunidad no encontrada.' });
      const unido = req.method === 'PUT';
      if (unido) await pool.query('INSERT INTO comunidad_miembros (comunidad_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [comunidadId, req.user.id]);
      else await pool.query('DELETE FROM comunidad_miembros WHERE comunidad_id = $1 AND user_id = $2', [comunidadId, req.user.id]);
      const count = await pool.query('SELECT COUNT(*)::int AS miembros FROM comunidad_miembros WHERE comunidad_id = $1', [comunidadId]);
      res.json({ unido, miembros: count.rows[0].miembros });
    } catch (error) {
    ...
    try {
      ...
      const result = await pool.query(`
        SELECT
          ...
          (SELECT COUNT(*) FROM comunidad_miembros cm WHERE cm.comunidad_id = c.id) AS miembros,
          EXISTS (SELECT 1 FROM comunidad_miembros cm WHERE cm.comunidad_id = c.id AND cm.user_id = $2) AS unido
        FROM comunidades c
        LEFT JOIN comunidad_publicaciones cp ON cp.comunidad_id = c.id
        WHERE c.activa = true
        GROUP BY c.id
        ORDER BY array_position($1::text[], c.id)
      `, [COMUNIDADES_GENERO.map((comunidad) => comunidad.id), await obtenerViewerId(req)]);
  ...
  crearComentario: async (req, res) => {
    ...
    try {
      ...
      const member = await pool.query('SELECT 1 FROM comunidad_miembros WHERE comunidad_id = $1 AND user_id = $2', [publicacion.rows[0].comunidad_id, req.user.id]);
      if (!member.rowCount) return res.status(403).json({ code: 'MEMBERSHIP_REQUIRED', error: 'Unite a la comunidad para responder.' });
