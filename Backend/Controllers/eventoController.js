...
async function asegurarEsquemaEventos() {
  if (!esquemaEventosListo) {
    esquemaEventosListo = (async () => {
      ...
      await pool.query('ALTER TABLE eventos ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT timezone(\'utc\'::text, now())');
      await pool.query(`
      ...
      `);
      await pool.query('CREATE INDEX IF NOT EXISTS idx_eventos_creador ON eventos(creador_id)');
      await pool.query('CREATE INDEX IF NOT EXISTS idx_eventos_fecha ON eventos(fecha)');
...
const eventoController = {
  ...
  crearEvento: async (req, res) => {
    ...
    if (!titulo || !genero || !ubicacion || !fecha || !Number.isFinite(latitudNormalizada) || !Number.isFinite(longitudNormalizada)) {
