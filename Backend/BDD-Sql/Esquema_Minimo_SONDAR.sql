...
CREATE TABLE public.eventos (
  ...
  genero text NOT NULL CHECK (lower(genero) IN ('pop', 'rock', 'edm', 'jazz', 'blues', 'cumbia', 'trap', 'metal', 'folklore', 'otros')),
  lugar text NOT NULL CHECK (length(trim(lugar)) BETWEEN 3 AND 240),
...
CREATE TABLE public.reels (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  titulo text NOT NULL CHECK (length(trim(titulo)) BETWEEN 1 AND 50),
  album text NOT NULL CHECK (length(trim(album)) BETWEEN 1 AND 60),
  genero text NOT NULL CHECK (lower(genero) IN ('pop', 'rock', 'edm', 'jazz', 'blues', 'cumbia', 'trap', 'metal', 'folklore', 'otros')),
  descripcion text NOT NULL DEFAULT '' CHECK (length(descripcion) <= 180),
...
CREATE TABLE public.comunidad_publicaciones (
  ...
  evento_asociado_id bigint REFERENCES public.eventos(id) ON DELETE SET NULL,
  reel_asociado_id bigint REFERENCES public.reels(id) ON DELETE SET NULL,
...
CREATE INDEX comunidad_publicaciones_evento_idx
  ON public.comunidad_publicaciones (evento_asociado_id) WHERE evento_asociado_id IS NOT NULL;
CREATE INDEX comunidad_publicaciones_reel_idx
...
CREATE TABLE public.content_reports (
  ...
  content_type text NOT NULL CHECK (content_type IN ('reel', 'evento', 'perfil')),
...
-- El cliente usa Supabase para Auth, no para DML de negocio. Todas las
-- escrituras pasan por el backend para aplicar validacion, cuotas y auditoria.
