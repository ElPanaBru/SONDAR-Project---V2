...
export default function Comunidad({ usuario }) {
  ...
  useEffect(() => {
    Promise.all([
      api.guardarCatalogo("comunidades", comunidadesIniciales),
      api.guardarCatalogo("hilos_iniciales", hilosIniciales),
    ]).catch((error) => console.error("No se pudo guardar el catalogo de comunidad:", error));
  }, []);
  ...
  const crearHilo = async (e) => {
    ...
    try {
      ...
      console.error(error);
      const hiloLocal = { id: crearIdLocal(), ...hiloPayload };
  ...
  const votar = async (id) => {
    if (!usuario?.uid) {
      alert("Inicia sesion para votar hilos.");
      return;
    }

    setHilos(hilos.map((hilo) =>
      hilo.id === id
        ? { ...hilo, votos: hilo.votado ? Math.max(hilo.votos - 1, 0) : hilo.votos + 1, votado: !hilo.votado }
        : hilo
    ));

    try {
      const data = await api.votarHilo(id, usuario.uid);
      setHilos((prev) => prev.map((hilo) =>
        hilo.id === id ? { ...hilo, votos: data.votos, votado: data.votado } : hilo
      ));
    } catch (error) {
      console.error(error);
      setHilos(hilos);
